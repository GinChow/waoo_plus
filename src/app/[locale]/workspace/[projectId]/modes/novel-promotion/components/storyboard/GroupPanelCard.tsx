...
// 把下一个分镜拉入本组的链接按钮；统一卡片布局下由列表渲染在外层卡片右缘
export function GroupLinkToNextButton({
  ...
  return (
    <button
      ...
      className={`
        ...
        ${linkEnableAllowed
          ? 'bg-[var(--glass-bg-surface)] text-[var(--glass-text-secondary)] hover:-translate-y-0.5 hover:shadow-[var(--glass-shadow-md)] hover:border-[var(--glass-stroke-focus)] hover:bg-[var(--glass-tone-info-bg)]'
  ...
  const content = (
    <>
      ...
      {/* 两张图组合：支持「首尾帧 / 多镜头」模式切换。
          统一卡片（embedded）下此开关由右侧视频制作区（VideoGroupPanelCard）提供，这里不再重复渲染 */}
      {!embedded && isTwoPanelGroup && projectId && storyboardId && firstPanel?.id && (
